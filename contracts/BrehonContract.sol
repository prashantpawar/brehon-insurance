...
contract BrehonContract is
  ...
  modifier eitherByParty(Party memory _party1, Party memory _party2)
  {
  ...
  modifier onlyByBrehon(Brehon memory _brehon) {
    ...
    eitherByParty(partyA, partyB)
  {
      ...
      if ((partyA.deposit + partyB.deposit) >=
          minimumContractAmt) {
             emit ExecutionStarted(msg.sender, partyA.deposit + partyB.deposit);
    ...
    eitherByParty(partyA, partyB)
  {
    ...
    emit ContractDisputed(msg.sender, primaryBrehon.addr);
  }
    ...
    onlyByBrehon(activeBrehon)
  {
    ...
    if (stage != Stages.Completed) {
        ...
        emit AppealPeriodStarted(appealPeriodStartTime, activeBrehon.addr, _awardPartyA, _awardPartyB);
    } else {
        stage = Stages.Completed;
    }
  }
    ...
    eitherByParty(partyA, partyB)
  {
    ...
    emit AppealRaised(msg.sender, activeBrehon.addr);
  }
    ...
    eitherByParty(partyA, partyB)
  {
    ...
    emit SecondAppealRaised(msg.sender, activeBrehon.addr);
  }
    ...
    eitherByParty(partyA, partyB)
  {
      ...
      emit SettlementProposed(msg.sender, _awardPartyA, _awardPartyB);
  }
    ...
    eitherByParty(partyA, partyB)
  {
      ...
      if(proposedSettlement.partyAAccepted && proposedSettlement.partyBAccepted) {
          ...
          stage = Stages.Completed; // STATECHANGE
          emit DisputeResolved(_awardPartyA, _awardPartyB);
      }
  }
}
